...
\begin{enumerate}
   ...
   \item For each formula \(\varphi \in \Phi\), for each symbol \(f\) occuring in \(\varphi\):

      An empty set of substitutions \(\Sigma_{\varphi, f}\) is created, then for every tuple \((\tau_1, \dots, \tau_n) \in  P(\varphi, f)\) and tuple \((\upsilon_1, \dots, \upsilon_n) \in M(f)\):

      \(\tau_i\) is matched against \(\upsilon_i\), possibly yielding the substitution \(\sigma_i\), for all \(i\). If all \(n\) matchings are successful and the substitutions are two by two compatible, \(\sigma_1 \circ \dots \circ \sigma_n\) is added to \(\Sigma_{\varphi, f}\).

   \item For each formula \(\varphi \in \Phi\), for each symbol \(f\) in \(\varphi\), for each substitution \(\sigma \in \Sigma_{\varphi, f}\), the formula \(\sigma(\varphi)\) is added to \(\Psi\).
...
An arbitrary number of iterations can be performed, re-using the output of one iteration as the input for the next. The final step requires all monomorphic formulae returned by the last iteration to be output.
...
This example illustrates how an infinite number of new formulae can be generated from a simple initial problem. Additionally, input problems are far larger and more explosive in practice and any reasonable implementation requires bounds to be useful.
