...
\textbf{ TODO rename non-monomorphic maps from \(P\) to \(N\) in high-level description section}
...
\begin{enumerate}
   ...
   \item[\labelitemi] \emph{match\((\upsilon, \tau)\)} with \(\upsilon\) and \(\tau\) types will match \(\upsilon\) against \(\tau\) and either fail or return \emph{Success\((\sigma)\)} where \(\sigma\) is the substitution resulting from the matching
...
The set of substitutions generated thusly is then iterated over and each substitution is applied to all polymorphic type arguments. The application of a substitution to a tuple of polymorphic type argument only takes place if least one type variable of a type in the tuple is instantiated by the substitution. This condition filters out all substitution application that would have otherwise left the type arguments unchanged.
...
Additionally, it is possible to either generalise or further refine these bounds. Indeed, the maximum number of newly generated type arguments is fixed per formula, per iteration. This algorithm can easily be modified such that the limit on new type arguments is fixed per iteration. Conversely, it can be made to limit new type arguments per symbol, per formula, per iteration.
...
To monomorphise a polymorphic formula, we first compute its \emph{monomorphising substitutions} (\ref{mono_substs}). A substitution \(\sigma\) is a monomorphising substitution for a formula \(\varphi\) if \(\sigma(\varphi)\) is monomorphic. It is a recursive function, given a set of type variables \(V\) and a pool of substitutions \(S\), it will select a substitution \(\sigma_{\negvthinspace\Delta}\) from \(S\) which instantiates at least one of the type variables in \(V\). It is important that \(\sigma_{\negvthinspace\Delta}\) be compatible with \(\sigma\) so that they may be composed and the function recursively called to instantiate the remaining type variables.
