...
Each bound presented presents a tradeoff, a higher bound will allow for a more exhaustive monomorphisation at the cost of taking longer to run. Our main goal being higher performance automatic theorem provers, we chose the number of problems solved in a 30 second time-frame as a measure of success of the monomorphisation procedure. To find the best bounds for this, it would be ideal to exhaustively test all possible combinations of values each bound could take. Given the number of bounds, this is not realistic. Instead, closely linked bounds are grouped together and their possible values are more thouroughly searched. Once the best performing set of values for a group of bounds is found, these values are chosen as a default and the search for the next group begins.
