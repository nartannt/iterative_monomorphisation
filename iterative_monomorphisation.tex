...
Table \ref{e_settings} evaluates the options with which we call the E prover. The call point corresponds to the fraction of the total timeout of Zipperposition's timeout after which the normal operation of Zipperposition is interrupted to begin the monomorphisation process. The E timeout limits how lond E is run before being interrupted and resuming the normal operation of Zipperposition. The longer Zipperposition is run, the more formulae are generated this and the explosive nature of iterative monomorphisation likely explains the poor performance of greater values of E call point.
...
Each of the two instances is evaluated on the set of the 500 previously used problems and the set of the remaining 1034 problems separately. The results of these evaluations seem to suggest that no significant overfitting took place during the option optimisation phase.

Table \ref{zipp_eval} shows that the use of E as a backend markedly improves the performance of Zipperposition. It is not a strict improvement as a number of problems are solved without E and not with E.
