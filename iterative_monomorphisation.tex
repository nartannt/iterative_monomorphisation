...
\Fn(){\IterMono{\(\Phi\)}}{
   ...
   \SetKwData{Problem}{\(\Phi\)}\SetKwData{AllSubst}{\(U\)}
   ...
   \For{\(1\leq i\leq \textcolor{ourblueviolet}{\Loop}\) }{
      ...
      \ForEach{\(\varphi \in \Problem\)} {
         \((M^*, P^*(\varphi), S)\leftarrow \FStep(M, P(\varphi), M^{\text{new}}, P^{\text{new}}(\varphi))\)\;
         \(\AllSubst\leftarrow\AllSubst\cup S\)\;
         \(M^{\text{next}}\leftarrow M^{\text{next}}\cup M^*\)\;
         \(P^{\text{next}}(\varphi)\leftarrow P^{\text{next}}(\varphi)\cup P^*(\varphi)\)\;
      }

      \((M, M^{\text{new}})\leftarrow (M\cup M^{\text{new}}, M^{\text{next}})\)\;
      \((P, P^{\text{new}})\leftarrow (P\cup P^{\text{new}}, P^{\text{next}})\)\;
...
%   \LinesNumbered
   ...
   \KwData{monomorphic type argument map \(M\), non-monomorphic type argument map \(P(\varphi)\)}
...
\begin{algorithm}[H]
   ...
   \KwData{old and new monomorphic type argument maps \(M, M^{\text{new}}\), old and new non-monomorphic type argument maps \(P(\varphi), P^{\text{new}}(\varphi)\)}
   \KwResult{monomorphic type argument map \(M^{\text{next}}\), non-monomorphic type argument map \(P^{\text{next}}(\varphi)\), set of substitutions \(S\)}
   \BlankLine
...
\begin{quote}
   ...
   \KwData{set of substitutions \(U\) and \(S\), single substitution \(\sigma\), set of type variables \(V\)}
   \KwResult{set of substitutions}
   \BlankLine
