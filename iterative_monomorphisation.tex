...
Notes:
   ...
   * In order to later have an easier time we store susbtitutions in a map that gathers them by which type variable they instantiate.

   * We assume the existence of "primitives" that compose substitutions, apply them to formulae ...

\begin{quote}
\begin{algorithm}[H]
   \caption{Clause Monomorphisation Step}
   ...
   \Input{\,monomorphic type argument map \(M, M'\), new non-monomorphic type argument map \(P(\varphi), P'(\varphi)\)}
   \Output{\,monomorphic type argument map, non-monomorphic type argument map, set of substitutions}
   \BlankLine
   ...
   \MaxMono \(\leftarrow\) \Max{\Min{\MonoFloor, \(|M| \times\)\MonoMult}, \MonoCap}\;
   \MaxPoly \(\leftarrow\) \Max{\Min{\PolyFloor, \(|P(\varphi)| \times\)\PolyMult}, \PolyCap}\;
   \BlankLine

   \UsedSubst \(\leftarrow\emptyset\)\;
   \ForEach{\(\sigma\in \Subst\)}{
      \ForEach{\(f\) occuring in \(\varphi\) }{
