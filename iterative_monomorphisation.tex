...
One approach \cite{mono-trans} is to encode a polymorphic type system using dedicated function or predicate symbols in a monomorphic logic. Another approach is to encode polymorphism using {iterative monomorphisation}, as described by B\"ohme~\cite[Section 2.2.1]{sb-phd}. This method heuristically instantiates the formulae's type variables with concrete types.
...
\relax{SMT-LIB} makes use of this monomorphisation approach \cite{smt-lib-mono}.
A similar algorithm also appears to be implemented in the \relax{MESON} tactic~\cite{harrison-1996} of HOL Light, but it is undocumented.
...
To illustrate the low level version of the iterative algorithm, we consider the following initial problem:
\begin{itemize}
...
%This step is unecessary if the target language allows type constructors.
