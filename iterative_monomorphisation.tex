...
The iterative monomorphisation algorithm (Figure~\ref{iter_mono}) has two phases. The first phase applies a \emph{monomorphisation step} (Figure~\ref{mono_step}) to each formula in \(\Phi\) until the user-set limit is reached. This limit is the only bound necessary for the algorithm to terminate. At the end of each of these iterations, the old and new type argument maps are updated with newly generated types. Once these iterations are completed, the first phase is completed and the substitutions used to create
new type arguments are passed to the formula generation function
(Figure~\ref{gen_formulae}) for the second phase.
...
Formula monomorphisation (Figure~\ref{mono_step}) computes new type arguments for a single formula. It forms the core of the monomorphisation process. The first step generates substitutions (Figure~\ref{subst_gen}) by matching the non-monomorphic type arguments against monomorphic ones. The separation of type arguments into old and new maps is put to use here. Pairs of type argument tuples must contain at least one new type argument to be matched. This avoids re-computing some of the matchings processed in previous iterations. Not all such redundant computations can be avoided because the same substitution can be obtained from different matchings.

The set of substitutions generated in this way is then iterated over.
Each substitution is applied to all polymorphic type argument tuples such that at least one type variable in the tuple is instantiated by the substitution. %This condition filters out all substitution application that would have otherwise left all type arguments unchanged.

The total number of type arguments can increase cubicly in the number of type arguments at each iteration and can therefore grow doubly exponentially in the number of iterations. We sketch an example of such growth.
%
If we assume that there are after \(k\) iterations \(N_k\) total type arguments split evenly between monomorphic and non-monomorphic type arguments, all in a single formula, then there can be up to \(\bigl(\frac{N_k}{2}\bigr)^2\) successful matches, yielding as many substitutions. Each substitution is then applied to each non-monomorphic type argument for a total of \(\bigl(\frac{N_k}{2}\bigr)^3\) possible new type argument tuples. If half of these new type arguments are monomorphic and the other half are non-monomorphic, then the next iteration will begin with \(N_{k+1} = \bigl(\frac{N_k}{2}\bigr)^{3}\) evenly split type argument tuples. Therefore, the total number of type arguments on the \(k\)th iteration can reach \(N_k = \bigl(\smash{\frac{N_0}{2}\bigr)^{3^k}} \cdot \bigl(\frac{1}{2}\bigl)^{3k}\).

Depending on the shape and size of the input problem and the number of iterations performed, the doubly exponential growth may be problematic. Introducing bounds addresses this potential issue. The limit on the number of new monomorphic type arguments (in \textcolor{ourblueviolet}{blue}) has three components:
\begin{enumerate}
   \item \textcolor{ourblueviolet}{\MonoCap} is an absolute limit on the total number of new type argument tuples.
   \item \textcolor{ourblueviolet}{\MonoMult} is used to allow the total number of type argument tuples to increase by a certain proportion of the current number of type argument tuples.
   \item \textcolor{ourblueviolet}{\MonoFloor} balances out \textcolor{ourblueviolet}{\MonoMult}. It prevents \textcolor{ourblueviolet}{\MonoMult} from inhibiting new type argument tuple generation if the total number of type argument tuples is low.
\end{enumerate}
