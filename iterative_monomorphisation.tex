...
The initial phase of each iteration consists in collecting the elements of the maps \(M\) and \(P\), defined as follows:
\begin{itemize}
    \item Given a type \(n\)-ary symbol \(f\) in \(\Phi\). \(M(f)\) is the set of all \(n\)-tuples of monomorphic type arguments to which \(f\) is applied in \(\Phi\).

    For example, if \(f\langle \tau_1, \tau_2, \tau_3\rangle\) (with \(\tau_1, \tau_2\) and \( \tau_3\) monomorphic) occurs in \(\Phi\) then, \((\tau_1, \tau_2, \tau_3) \in M(f) \).

 \item Given a formula \(\varphi \in \Phi\) and a type \(n\)-ary symbol \(f\) in \(\varphi\). \(P(\varphi, f)\) is the set of all \(n\)-tuples of type arguments to which \(f\) is applied in \(\varphi\). At least one of these type arguments must be polymorphic.
    
    For example, if \(f\langle \tau_1, \tau_2, \tau_3\rangle\) (with at least one of \(\tau_1, \tau_2\) or \( \tau_3\) polymorphic) occurs in \(\varphi\) then, \((\tau_1, \tau_2, \tau_3) \in P(\varphi, f) \).
...
A single iteration will consist in the following:
\begin{itemize}
   ...
   \item For each formula \(\varphi \in \Phi\), for each symbol \(f\) in \(\varphi\):
      ...
      An empty set of substitutions \(\Sigma_{\varphi, f}\) is created, then, for each type argument tuple \((\tau_1, \dots, \tau_n) \in  P(\varphi, f)\) and each type argument tuple \((\upsilon_1, \dots, \upsilon_n) \in M(f)\):

      Each \(\tau_i\) is matched against its corresponding \(\upsilon_i\) possibly yielding the substitution \(\sigma_i\).
      If all \(n\) matchings are successful and the substitutions are two by two compatible, \(\sigma_1 \circ \dots \circ \sigma_n\) is added to \(\Sigma_{\varphi, f}\).
...
After a set number of iterations, all monomorphic formulae are collected from \(\Phi\) and output.
...
\begin{enumerate}
   \item \(\forall a: \alpha, b:\ty{list}(\alpha), f\langle\alpha,\ty{list}(\alpha)\rangle(a, b)\)
   \item \(\forall i: \ty{int}, j: \ty{list}(\ty{int}), f\langle \ty{int}, \ty{list}(\ty{int})\rangle(i, j)\)
\end{enumerate}
