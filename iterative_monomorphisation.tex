...
This algorithm relies on ``primitives'' whose implementation depend on the specifics of the grammar and logic used. They will therefore not be expanded upon. Functions computing the following are assumed to be available: 
\begin{enumerate}
   \item[\labelitemi] extracting the initial type argument maps from the input problem
   \item[\labelitemi] gathering type variables from a type tuple
...
\Fn(){\(\IterMono(\Phi)\)}{
   ...
   \KwData{finite set \(\Phi\) of polymorphic formulae}
   \KwResult{finite set of monomorphic formulae}

   \BlankLine

   \((M_{\text{old}}, P_{\text{old}}) \leftarrow (\emptyset, \emptyset)\)\;
   \((M_{\text{new}}, P_{\text{new}}) \leftarrow \Init(\Problem)\)\;
   ...
   \For{\(i = 1\) \To \textcolor{ourblueviolet}{\Loop}}{
      \((M_{\text{next}}, P_{\text{next}})\leftarrow(\emptyset,\emptyset)\)\;
      \ForEach{\(\varphi \in \Problem\)} {
         \((M_{\Delta}, P_{\Delta}(\varphi), S_{\Delta})\leftarrow \FStep(M_{\text{old}}, P_{\text{old}}(\varphi), M_{\text{new}}, P_{\text{new}}(\varphi))\)\;
         \(\AllSubst\leftarrow\AllSubst\cup S_{\Delta}\)\;
         \(M_{\text{next}}\leftarrow M_{\text{next}}\cup M_{\Delta}\)\;
         \(P_{\text{next}}(\varphi)\leftarrow P_{\text{next}}(\varphi)\cup P_{\Delta}(\varphi)\)\;
      }

      \((M_{\text{old}}, M_{\text{new}})\leftarrow (M_{\text{old}}\cup M_{\text{new}}, M_{\text{next}})\)\;
      \((P_{\text{old}}, P_{\text{new}})\leftarrow (P_{\text{old}}\cup P_{\text{new}}, P_{\text{next}})\)\;
...
\Fn(){\(\GenFormulae(\Phi, S)\)}{
   ...
   \KwData{\begin{minipage}[t]{.8\textwidth}
     \strut finite set \(\Phi\) of polymorphic formulae \\
     finite set $S$ of substitutions\strut
     \end{minipage}}
   \KwResult{finite set of monomorphic formulae}
   ...
   \ForEach{\(\varphi\in\Phi\) such that $\varphi$ is non-monomorphic}{
...
The iterative monomorphisation algorithm (\ref{iter_mono}), as implemented in Zipperposition, has two main phases each consisting of a separate loop. The first loop applies a \emph{monomorphisation step} to each formula (\ref{mono_step}) for each iteration of the outermost for loop until the user-set limit it reached. This bound is the only limit which is necessary for the algorithm to terminate. At the end of each of these iterations, the old and new type argument maps are updated with newly generated types.
...
Once the first phase is completed, the substitutions used to create new type arguments are combined to form \emph{monomorphising substitutions}. These substitutions are subsequently applied to the input problem's polymorphic formulae. The possible number of new (monomorphic) formulae resulting from this process is high but can be limited by setting a maximal number of new formulae. In the Zipperposition implementation, this bound is one of the more important ones as the performance of the E prover can be significantly affected by the number of clauses it is given.
...
\begin{algorithm}[H]
   ...
   \Fn(){\FMonoStep{\(M_{\text{old}}, M_{\text{new}}, P_{\text{old}}(\varphi), P_{\text{new}}(\varphi)\)}}{
   ...
   \KwData{\begin{minipage}[t]{.8\textwidth}
     \strut old and new monomorphic type argument maps \(M_{\text{old}}, M_{\text{new}}\) \\
     old and new non-monomorphic type argument maps \(P_{\text{old}}(\varphi), P_{\text{new}}(\varphi)\)\strut
      \end{minipage}
   }
   ...
   \MaxPoly \(\leftarrow\) \Max{\Min{\PolyFloor, \(|P_{\text{old}}(\varphi)\cup P_{\text{new}}(\varphi)| \cdot \PolyMult\)}, \PolyCap}\;
   }
   ...
   \ForEach{\(\sigma\in {}\)\SubstGen{\(M_{\text{new}}\), \(P_{\text{new}}(\varphi)\)}
\(\cup\) \SubstGen{\(M_{\text{new}}\), \(P_{\text{old}}(\varphi)\)}
\(\cup\) \SubstGen{\(M_{\text{old}}\), \(P_{\text{new}}(\varphi)\)}}{
      \ForEach{\(f\mapsto (\upsilon_1, \dots,\upsilon_n)\in P_{\text{old}}(\varphi)\)}{
            \eIf{\TyVars{\((\upsilon_1, \dots, \upsilon_n)\)} \(\subseteq\) \Domain{\(\sigma\)}}{
               \textcolor{ourblueviolet}{
               \uIf{\(|M_{\text{next}}| < \MaxMono\) \label{algo:relative_bound}}{
                  \textcolor{black}{
                  \(M_{\text{next}}(f)\leftarrow M_{\text{next}}(f)\cup\{(\sigma(\upsilon_1),\dots,\sigma(\upsilon_n))\}\)\;
                  \UsedSubst \(\leftarrow \UsedSubst \cup\{\sigma\}\)\;}
               }
               \uElseIf{\(|P_{\text{next}}(\varphi)| \geq \MaxPoly\) }{
                  \(M_{\text{next}} \leftarrow M_{\text{next}}\setminus (M_{\text{old}} \cup M_{\text{new}})\)\;
                  \(P_{\text{next}}(\varphi)\leftarrow P_{\text{next}}(\varphi)\setminus (P_{\text{old}}(\varphi)\cup P_{\text{new}}(\varphi))\)\;
                  \Return \((M_{\text{next}}, P_{\text{next}}(\varphi), \UsedSubst)\)
               }}
            }
            {
               \textcolor{ourblueviolet}{
               \If{\(|P_{\text{next}}(\varphi)| < \MaxPoly\)\label{algo:ref_2}} {
               \textcolor{black}{
               \(P_{\text{next}}(\varphi)(f)\leftarrow P_{\text{next}}(\varphi)(f)\cup\{(\sigma(\upsilon_1),\dots,\sigma(\upsilon_n))\}\)\;
               \UsedSubst \(\leftarrow \UsedSubst \cup\{\sigma\}\)\;
            }}
            }
   ...
   \Return \((M_{\text{next}}\setminus (M_{\text{old}} \cup M_{\text{new}}), P_{\text{next}}(\varphi)\setminus (P_{\text{old}}(\varphi)\cup P_{\text{new}}(\varphi)), \UsedSubst)\)
}
...
Formula monomorphisation (\ref{mono_step}) computes new type arguments for a single formula, it is the core of the monomorphisation process. The first step generates substitutions (\ref{subst_gen}) by matching the non-monomorphic type arguments against monomorphic ones. The separation of the type arguments into ``old'' and ``new'' maps is exploited here. Pairs of type argument tuples must contain at least one new type argument in order to be matched. This avoids re-computing some matchings already processed in previous iterations. Not all such redundant computations can be avoided because the same substitution can be obtained from matchings between entirely different pairs of types.

The set of substitutions generated thusly is then iterated over and each substitution is applied to all polymorphic type arguments. The application of a substitution to a polymorphic type argument will only take place if any type variable of the type argument is instantiated by the substitution. This condition filters out all substitution application that would have left the type arguments unchanged.
...
The total number of type arguments can increase cubicly in the number of type arguments at each iteration and can therefore grow doubly exponentially in the number of iterations. Depending on the shape and size of the input problem and the number of iterations performed, this may be problematic. The limit on the number of new monomorphic type arguments (in \textcolor{ourblueviolet}{blue}) has three components (similar bounds are used for the non-monomorphic type arguments):
\begin{enumerate}
...
Additionally, it is possible to either generalise or further refine these bounds. Indeed, the maximum number of newly generated type arguments is fixed per formula, per iteration. This algorithm can easily be modified such that the limit on new type arguments is fixed per iteration, or conversely, be made to limit new type arguments per symbol, per formula, per iteration.
...
Substitution generation (\ref{subst_gen}) consists in computing all possible matchings between the type arguments of the given type argument maps and returns the resulting substitutions. For each pair of type argument tuples, the non-monomorphic types are matched against their monomorphic counterparts and the resulting substitutions are composed if they are compatible. In the pseudocode, this is checked by making sure that the while loop has iterated over all elements of the type argument tuple. If the substitutions are incompatible, the matchings are discarded. [Note that the composition of two compatible substitutions is commutative and that the order of composition is therefore irrelevant]. [The Zipperposition implementation is written in a functional style and does not directly map onto the pseudocode].

The various bounds presented here are, to some extent, redundant. For instance, limiting the number of substitutions generated at this step may be sufficient to curb the number of new type arguments, making the related bounds superfluous. Each bound nonetheless has its use cases. For example, sets of problems which lead to few successful matches but many type arguments may benefit from a limit on the number of new type arguments whilst sets of problems where substitution generation is a bottleneck might consider a limit on the number of generated substitution to be more useful. 
...
\begin{quote}
   ...
   \Fn(){\MonoSubst{\(S_{\text{all}}, S^\text{acc}, \sigma^\text{acc}, V\)}}{

   \KwData{%
      \hspace{-2.7em}\begin{minipage}[t]{.8\textwidth}
      \begin{enumerate}
         \item[] set of substitutions \(S_{\text{all}}\) and \(S^\text{acc}\)
   ...
   }
   {
      ...
      \ForEach{\(\sigma_{\text{next}}\in S_{\text{all}}\) such that, \(\alpha\in\Domain{\(\sigma_{\text{next}}\)}\) \And \(\sigma^\text{acc}\) compatible with \(\sigma_{\text{next}}\)}{
         \textcolor{ourblueviolet}{
         \eIf{\(|S^\text{acc}|<\Limit\)}{
            \textcolor{black}{
            \(S^\text{acc}\leftarrow \MonoSubst{\(S_{\text{all}}, S^\text{acc}, \sigma_{\text{next}}\circ\sigma^\text{acc}, V\setminus\Domain{\(\sigma_{\text{next}}\)}\)}\)\;}
         }
         {\Return \(S^\text{acc}\)}}
      }
      \Return \(S^\text{acc}\)\;
...
Once all monomorphisation iterations have been completed, we are left with a set of substitutions. The last phase uses these substitutions to instantiate the type variables in the input problem's polymorphic formuale.

To monomorphise a polymorphic formula, we first compute its \emph{monomorphising substitutions} (\ref{mono_substs}). A substitution \(\sigma\) is a monomorphising substitution for a formula \(\varphi\) if \(\sigma(\varphi)\) is monomorphic. \ref{mono_substs} is a recursive function, given a set of type variables \(V\) and a pool of substitutions \(S^\text{all}\), it will select a substitution \(\sigma_{\text{next}}\) from \(S^\text{all}\) which instantiates at least one of the type variables in \(V\). It is important that \(\sigma_{\text{next}}\) be compatible with \(\sigma^\text{acc}\) so that they may be composed and the function recursively called to instantiate the remaining type variables.

The Zipperposition implementation of this function makes use of a map from type variables to substitutions in place of a simple set to filter the relevant substitutions from \(S^\text{all}\) with greater ease. The \textcolor{ourblueviolet}{subst\_limit} exists for two main reasons:
\begin{enumerate}
   \item The iterative monomorphisation algorithm can generate up to \textcolor{ourblueviolet}{subst\_limit} new monomorphic clause per initial polymorphic clause. Generating an excessive number of new clauses can lead to the downstream prover being flooded.
   \item This function is the most explosive part of the algorithm. If \(S^\text{all}\) contains \(n\) substitutions which each instantiate exactly one of \(v\) different type variable, up to \(n^v\) monomorphising substititions may be generated. This should be considered in light of the fact that the total number of type variables (used to generate these substitutions) is already doubly exponential in the number of loop iterations.
\end{enumerate}

%[This function can be thought of as exploring the tree of all possible monomorphising substitutions, \(S^\text{acc}\) would represent the leaves that have already been visited and \(\sigma^\text{acc}\) would represent the current node of the tree.]

Once the newly generated monomorphic formulae are output, an extra post-processing step may be required. Indeed, monomorphic provers, such as E do not necessarily support type constructors. Consequently, transforming the monomorphic formulae into equivalent ones without type constructors may be necessary as it was for the Zipperposition implementation. This transformation is largely syntactic, it is documented in [insert names] 's paper [insert citation].
