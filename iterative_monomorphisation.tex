...
The monomorphisation algorithm is paramatrised by many bounds, the first step of the evaluation process is to find appropriate values for these bounds. The second step compares the performance of Zipperposition with and without monomorphisation. Finally, the third step tests the relative performance of different provers on polymorphic problems and their monomorphised counterparts.

The problems used for these tests are taken from the TPTP library. The TPTP library contains 1765 problems in the TF1 and TH1 grammars, they are the first-order and higher-order problems with rank-1 polymorphism. Because Zipperposition does not support reasoning with real numbers, we removed all problems including real numbers or importing axioms including real numbers. In total our testing suite contains 1754 polymorphic problems. For the first step, we chose as a measure of success the number of problems solved by that instance in under 30 seconds each with a single thread. This setup mirrors the setting of interactive provers in which an instance of an automatic prover will typically be called for similar durations. As a precaution against overfitting, all tests of this phase are made on 500 problems randomly taken out of the 1754 total problems. 

\subsection{Parameter optimisation}
\label{param_opti}

Before finding values to assign to the bounds of the monomorphisation algorithm, we must make the choice of the base options with which Zipperposition will be instantiated. Zipperposition has a portfolio mode with several different configurations, we evaluated all base configurations which called E against our test set of 500 problems. The best performing configuration became the basis upon which we tested the different monomorphisation options. Our evaluation found that the ``40 e lift'' configuration performed best by solving 136 problems in under 30 seconds each.%[TODO recheck this]

Each bound of the monomorphisation process presents a tradeoff: a higher bound allows for a more exhaustive instantiations of type variables at the cost of taking more time. Ideally, all possible combinations of values for all bounds could be exhaustively tested to find the best compromise between completeness and speed. Given the number of bounds, however, this is not feasible. Instead, we group closely related bounds together and test combinations of values for bounds in these groups. Once we find the best performing set of values for a group of bounds, we assign these values to the corresponding bounds as we begin the search for the next group. This method requires an initial default set of bounds and reasonable ranges of values for each bound. We chose these values based on the results of preliminary evaluations.

Table~\ref{mono_ty_args} groups bounds that influence the maximum number of newly generated monomorphic type arguments per clause per iteration. The limit of newly generated type arguments per clause is determined by three components which form a natural group of bounds. Our results show that generating no new monomorphic type arguments seems to be the best approach. It is possible to monorphise formuale without generating monomorphic type arguments because non-monomorphic type argument generation can generate substitutions which instantiate one or more type variables and it is these substitutions which are used to monomorphise formulae.
...
\centering\begin{tabular}{@{}l*{12}{>{\centering\arraybackslash}p{1.5em}}@{}}
   ...
   \multirow{1}{2em}{mult} & 0 & 50 & 100 & 200& 0 & 50 & 100 & 200& 0 & 50 & 100 & 200\\
    ...
    0         & 170 & 151 & 146 & 136 & 170 & 151 & 144 & 137 &\bf{171}&149&137 & 132\\
    1         & 130 & 130 & 133 & 132 & 131 & 131 & 131 & 131 & 131 & 131 & 131 & 130\\
    2         & 130 & 131 & 131 & 132 & 131 & 131 & 132 & 130 & 129 & 130 & 130 & 130\\
    \(\infty\)& 131 & 132 & 131 & 131 & 131 & 131 & 130 & 131 & 145 & 144 & 146 & 147\\
    \bottomrule
\end{tabular}
\label{mono_ty_args}
\end{table}

Table~\ref{nmon_ty_args} fills the same role except it is for bounds limiting the number of new non-monomorphic type arguments. The bound values are lower because in preliminary evaluation we found non-monomorphic type arguments to be explosive. The results confirm that it is non-monomorphic argument generation which drives the creation of useful non-monomorphic clauses. This is indicatied by the very low number of problems solved when no new non-monomorphic type arguments are allowed. Additionally, we observe that removing all bounds considerably reduces performance. %TODO check that 124 is what we get if we just run Zipperposition for 10 seconds (maybe with a few extra problems that are easily monomorphised)
...
\centering\begin{tabular}{@{}l*{12}{>{\centering\arraybackslash}p{1.5em}}@{}}
   ...
   \multirow{1}{3em}{mult} & 0 & 10 & 50 & 100& 0 & 10 & 50 & 100& 0 & 10 & 50 & 100\\
    \cmidrule(lr){2-5} \cmidrule(lr){6-9} \cmidrule(l){10-13} 
    0          & 124 & 170 & 171 & 170 & 124 & 170 & 171 & 171 & 124 & 170 & 171 & 170\\
    0.5        & 165&171&\bf{172}& 170 & 165 & 171 & 171 & 170 & 165 & 171 & 171 & 170\\
    1          & 170 & 171 & 171 & 171 & 170 & 171 & 171 & 171 & 170 & 171 & 171 & 171\\
    \(\infty\) & 171 & 171 & 171 & 171 & 168 & 169 & 167 & 168 & 124 & 124 & 124 & 124\\
    \bottomrule
\end{tabular}
\label{nmon_ty_args}
\end{table}

The substitution generation phase occurs once all type arguments have been generated. The bounds limiting the number of new substitutions and the number of monomorphising substitutions per formula are directly related as the construction of monomorphising substitutions makes use of all available substitutions. The substitution ordering heuristic has a significant impact on monomorphising substitution generation. The ``age'' ordering of substitution orders substitutions based on which iteration they were generated in, earlier substitutions are ordered first. The ``random'' ordering randomly shuffles substitutions. Finally, the ``segregation'' ordering separates the substitutions into different sets depending on which iteration they were generated in and generates monomorphising substitutions from each of these sets independently. Table \ref{subst_gen_table} shows that the specific choice of bounds limiting the number of substitutions generated has very little impact on overall performance contrary to the ordering heuristic.
\begin{table}[th]
...
\centering\begin{tabular}{@{}l*{12}{>{\centering\arraybackslash}p{1.5em}}@{}}
   ...
   \cmidrule(lr){2-5}\cmidrule(lr){6-9}\cmidrule(l){10-13}
   50         & 158 & 158 & 158 & 158& 169 & 169 & 169 & 169& 167 & 170 & 170 & 171\\
   100        & 158 & 158 & 158 & 158& 169 & 169 & 169 & 169& 167 & 170 & 170 & 171\\
   200        & 158 & 158 & 158 & 158& 169 & 169 & 169 & 169& 167 & 171 & 170 & 171\\
   \(\infty\) & 158 & 158 & 158 & 158& 169 & 169 & 169 & 169& 167 & 170 & 170 & 171\\
...
Table \ref{pb_size} groups together the bounds related to the size of the output problem to be passed to the E prover. The absolute cap refers to the maximum total number of formulae passed to E. The multiplier instead limits the total number of newly generated formulae based on the problem's initial number of formuale. The performance of the E prover is highly dependent on the number of formulae it is given. To account for this, we test the effects of the amount of time allocated to E on the monomorphised problem against the size of the monomorphised problem.
...
Table \ref{mono_time} tests the amount of time that monomorphisation is allowed to run for before being interrupted and returning to the unmodified operation of Zipperposition against the number of iteration of the monomorphisation algorithm. A higher loop number will lead to longer monomorphisation time, we therefore expect better performance whenever loop number and monomorphisation time have coherent values.
...
\centering\begin{tabular}{@{}l*{4}{>{\centering\arraybackslash}p{1em}}@{}}
   ...
   \multirow{1}{4.5em}{loop nb} & 5 & 10 & 20 & 30\\
   \midrule
   2 & 0 & 0 & 0 & 0\\
   3 & 0 & 0 & 0 & 0\\
   4 & 0 & 0 & 0 & 0\\
