...
\address[1]{Departement d'informatique, École normale supérieure Paris-Saclay, France} % TODO dpt info
...
% We have two options, either use the current tables and make a tiered old/new test structure, or test everyting at once, which would be possible (32 options vs 16 with the tiered approach) however the results would most likely not be very pleasant to present, a highlight would have to be selected.

% final tests to make sure everything plays well together
\begin{table}[ht]
