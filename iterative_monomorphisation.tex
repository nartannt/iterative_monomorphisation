...
This algorithm does not need to inspect the structure of the problem's formulae because it relies exclusively on the formulae's monomorphic and polymorphic symbol instances. The notion of formula is necessary as type variables are assumed to be universally quantified at a formula's top level. The precise form of a formula can nevertheless be left unspecified.

Iterative monomorphisation can therefore be used in a vast range of rank-1 polymorphic logic. In particular, it can operate in the polymorphic first- and higher-order logics embodied by TPTP's TF1 and TH1 syntaxes \cite{blanchette-paskevich-2013,th1}, popular among automatic provers.
...
A \emph{symbol instance} refers to a symbol provided with its type arguments.
\end{definition}

\begin{definition} % HEAVILY inspired from 2.4 in mono-trans (it is the definition of a substitution, there's bound to be some comonality between all of them) 
A \emph{substitution} \(\sigma\) is a function mapping one or more type variables to corresponding monomorphic type. Substitutions are in practice mostly be applied to formulae. Given a formula \(\Phi\), the formula obtained by replacing each type variable \(\alpha\) in \(\Phi\) by \(\sigma\alpha\) is written \(\sigma\Phi\).

\end{definition}

\begin{definition}
   Given a monomorphic type \(\tau_1\) and a polymorphic type \(\tau_2\), \emph{matching} \(\tau_2\) against \(\tau_1\) will either fail or yield a substitution \(\sigma\) such that \(\sigma\tau_2 = \tau_1\). Note that in the second case \(\sigma\tau_2\) is monomorphic.
\end{definition}
