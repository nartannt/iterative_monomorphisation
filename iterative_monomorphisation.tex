...
\begin{quote}
   ...
   \Fn(){\MonoSubst{\(S^{\text{all}}, S^\text{acc}, \sigma^\text{acc}, V\)}}{

   \KwData{%
      \hspace{-2.7em}\begin{minipage}[t]{.8\textwidth}
      \begin{enumerate}
         \item[] set of substitutions \(S^{\text{all}}\) and \(S^\text{acc}\)
   ...
   }
   {
      ...
      \ForEach{\(\sigma^\text{next}\in S^{\text{all}}\) such that, \(\alpha\in\Domain{\(\sigma^\text{next}\)}\) \And \(\sigma^\text{acc}\) compatible with \(\sigma^\text{next}\)}{
         \textcolor{ourblueviolet}{
         \eIf{\(|S^\text{acc}|<\Limit\)}{
            \textcolor{black}{
            \(S^\text{acc}\leftarrow \MonoSubst{\(S^{\text{all}}, S^\text{acc}, \sigma^\text{next}\circ\sigma^\text{acc}, V\setminus\Domain{\(\sigma^\text{next}\)}\)}\)\;}
         }
         {\Return \(S^\text{acc}\)}}
      }
      \Return \(S^\text{acc}\)\;
