...
\begin{algorithm}
   ...
   \If{\(V = \emptyset\) \And \(|S| < \Limit\)}{
      \Return \(S\cup\{\sigma\}\)\;
   }
   \Else{
      \(\exists\alpha\in V\)\;
      \ForEach{\(\sigma'\in U\) such that \(\sigma'\) instantiates \(\alpha\) \And \(\sigma\) compatible with \(\sigma'\)}{
         \If{\(|S|<\Limit\)}{
...
\begin{algorithm}
   ...
   \For{\(\varphi\in\Phi\)}{
      ...
      \ForEach{}{
         \(\NewFormulae\leftarrow\NewFormulae\cup\)
      }
