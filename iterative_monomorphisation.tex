...
The iterative monomorphisation algorithm (Figure~\ref{iter_mono}) has two phases. The first phase applies a \emph{monomorphisation step} (Figure~\ref{mono_step}) to each formula in \(\Phi\) until the user-set limit, \textcolor{ourblueviolet}{num\_loops}, is reached. This limit is the only bound necessary for the algorithm to terminate. We use the colour blue to identify bounds and for all code that is related to bounds. At the end of each of these iterations, the old and new type argument maps are updated with newly generated types. Once these iterations are completed, the first phase is completed and the substitutions used to create
...
\SetKwData{PolyCap}{\textcolor{ourblueviolet}{nonmono\_cap}}
\SetKwData{PolyMult}{\textcolor{ourblueviolet}{nonmono\_mult}}
\SetKwData{PolyFloor}{\textcolor{ourblueviolet}{nonmono\_floor}}
\SetKwData{MaxMono}{\textcolor{ourblueviolet}{max\_mono\_args}}
\SetKwData{MaxPoly}{\textcolor{ourblueviolet}{max\_nonmono\_args}}
...
%This algorithm can easily be modified such that the limit on new type arguments is fixed per iteration. Conversely, new type arguments can be limited per symbol, per formula, per iteration.
...
Substitution generation (Figure~\ref{subst_gen}) consists of computing all possible matchings between the type arguments of the given type argument maps and returns the resulting substitutions. For each pair of type argument tuples, the non-monomorphic types are matched against their monomorphic counterparts, and the resulting substitutions are composed if they are compatible. In the pseudocode, this is checked by making sure that the \textbf{while} loop has iterated over all elements of the type argument tuple. If any substitutions are incompatible, the matchings are discarded. Since the composition of two compatible substitutions is commutative, the order of composition is irrelevant.

The various bounds presented here overlap to some extent. For instance, limiting the number of substitutions generated at this step may be sufficient to curb the number of new type argument tuples, making the \textcolor{ourblueviolet}{\MonoCap}, \textcolor{ourblueviolet}{\MonoMult}, \textcolor{ourblueviolet}{\MonoFloor} triplet superfluous. Each bound nonetheless has its use cases. For example, problems that lead to few successful matches but many type arguments may benefit from a limit on the number of new type arguments whilst sets of problems where substitution generation is more explosive might consider a limit on the number of generated substitution to be more useful. 
