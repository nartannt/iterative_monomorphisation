...
\begin{enumerate}
   ...
   \item \(\forall i: \ty{int}, f\langle \ty{int}\rangle(i)\)
\end{enumerate}
...
\centering\begin{tabular}{@{}lccccccc@{}}
   ...
   \midrule
   \(M\)       & \((\ty{int})\)                     & -                     & \((\ty{list}(\ty{int}))\) \\
   \(P(1, f)\) & \((\alpha)\; (\ty{list}(\alpha))\) & -                     & - \\
   \(P(2, f)\) & \(\emptyset\)                      & -                     & - & \(\emptyset\) \\
   \(\Phi\)    & [1] \; [2]                         & \(\sigma(\text{[1]})\)& \(\sigma'(\text{[1]})\)& \\
   \bottomrule
\end{tabular}
\end{table}
...
At the beginning of the second iteration, the clause generated in the previous iteration is used to update \(M\).
