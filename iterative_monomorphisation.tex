...
\textbf{TODO: Mention that all sets referenced in the algorithm are finite.}
...
\Fn(){\(\GenFormulae(\Phi, S)\)}{
   ...
   \ForEach{\(\varphi\in\Phi\) \textbf{such that} $\varphi$ is non-monomorphic}{
...
\(\cup\) \SubstGen{\(M_{\text{old}}\), \(N_{\text{new}}(\varphi)\)}}{
      \ForEach{\(f\mapsto (\upsilon_1, \dots,\upsilon_n)\in N_{\text{old}}(\varphi)\)}{
            ...
            {
               \textcolor{ourblueviolet}{
               \If{\(|N_{\text{next}}(\varphi)| < \MaxPoly\)\label{algo:ref_2}} {
               \textcolor{black}{
...
\begin{quote}
   ...
   {
      \Let \(\alpha \) \textbf{such that} \(\alpha\in V\)\;
      \ForEach{\(\sigma_{\negvthinspace\Delta}\in S\) \textbf{such that} \(\alpha\in\Domain{\(\sigma_{\negvthinspace\Delta}\)}\) \And \(\Compatible(\sigma,\sigma_{\negvthinspace\Delta})\)}{
         \textcolor{ourblueviolet}{
...
The Zipperposition implementation of this function makes use of a map from type variables to substitutions in place of a simple set to filter the relevant substitutions from \(S^\text{all}\) with greater ease. The \textcolor{ourblueviolet}{max\_substs} exists for two main reasons:
\begin{enumerate}
