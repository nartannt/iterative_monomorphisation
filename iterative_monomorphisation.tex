...
\begin{algorithm}[H]
   ...
   \textcolor{ourblueviolet}{
   \MaxMono \(\leftarrow\) \Max{\Min{\MonoFloor, \(|M\cup M'| \cdot \)\MonoMult}, \MonoCap}\;
   \MaxPoly \(\leftarrow\) \Max{\Min{\PolyFloor, \(|P(\varphi)\cup P'(\varphi)| \cdot \)\PolyMult}, \PolyCap}\;
   }
   ...
   \ForEach{\(\sigma\in \Subst\)}{
      \ForEach{\(f\mapsto (\upsilon_1, \dots,\upsilon_n)\in P(\varphi)\)}{
         {
            \uIf{\TyVars{\((\upsilon_1, \dots, \upsilon_n)\)} \(\subseteq\) \Domain{\(\sigma\)}}{
               \textcolor{ourblueviolet}{
               \uIf{\(|M''| < \MaxMono\) \label{algo:relative_bound}}{
                  \textcolor{black}{
                  \(M''(f)\leftarrow M''(f)\cup\{(\sigma(\upsilon_1),\dots,\sigma(\upsilon_n))\}\)\;
                  \UsedSubst \(\leftarrow \UsedSubst \cup\{\sigma\}\)\;}
               }
               \uElseIf{\(|P''(\varphi)| \geq \MaxPoly\)}{
                  \Return \((M'', P''(\varphi), \UsedSubst)\)\;
               }}
            }
            \uElse{
               \textcolor{ourblueviolet}{
               \If{\(|P''(\varphi)| < \MaxPoly\)\label{algo:ref_2}} {
               \textcolor{black}{
