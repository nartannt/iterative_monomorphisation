...
\begin{abstract}
Monomorphisation makes it possible to extend monormophic provers to support
polymorphic logics. We propose an iterative approach, which is necessarily
incomplete but which works well in practice. It is implemented in the
Zipperposition prover, where it can be used to translate away polymorphism
before invoking a monomorphic prover. Our empirical evaluation reveals that
iterative monorphisation is a viable alternative to native polymorphism.
\end{abstract}
...
%\cite{mono-trans}
%\cite{expr-poly-types}
%\cite{sb-phd}

%what we are trying to do (monomorphisation)\\
This paper presents an iterative method, first introduced in \cite{sb-phd} (section 2.2.1), that allows for higher order polymorphic problems to be approximated monomorphically. This algorithm aims to achieve the following:
   \begin{itemize}
      \item extend higher order theorem provers to polymorphic languages
      \item improve the performance of existing native polymorphic theorem provers
   \end{itemize}

This algorithm is implemented in Zipperpostion, a higher order polymorphic automatic theorem prover written in OCaml. As a result, the E prover, which does not support polymorphism, can be used as a backend to Zipperposition. The viability of this approach is then evaluated, both as an improvement to Zipperposition and as a means to increase the performance of other provers.
...
we assume problems to be sets of clauses. All type arguments of the problem are collected and the polymorphic type arguments are matched against the monomorphic ones. This yields new type arguments, both polymorphic and monomorphic. The process is then repeated making use of the newly derived type arguments.

\smallskip

These iterations are required in order to account for inductively defined types. Consider the type constructor \verb|list| which takes a type argument, if we obtain \verb|list(alpha)| where \verb|alpha| is a type variable, it may be possible to generate the types \verb|list(int)|, \verb|list(list(int))|, ... However because new types are obtained through matching, \verb|list(list(int))| can only be obtained once \verb|list(int)| has already been added to the set of monomorphic types. This example also makes it clear that the set of types that can be generated is infinite, this explains why the number of iterations is bounded.

\smallskip

After a set number of iterations, the new type arguments are used to instantiate the type variables in the problem's clauses, generating new monomorphic clauses.
   
Finally, we ``mangle'' the types in the output clauses using the method presented in \cite{mono-trans}, in order to remove type constructors from the output problem.

This approach sacrifices completeness for the sake of performance. The loss of completeness occurs at the level of the algorithm; a finite number of iterations and at the implementation level; the explosive nature of the procedures requires various bounds which limit the number of type arguments and clauses that are generated.
