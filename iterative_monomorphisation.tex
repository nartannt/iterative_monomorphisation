...
By a type version of the compactness theorem, we have that for any first order polymorphic formula \(\varphi\), there exists an equisatisfiable finite set of monomorphic instances of \(\varphi\). However, this set cannot be computed~\cite[Theorem~1]{expr-poly-types}.
...
\begin{enumerate}
   ...
   \item[\labelitemi] \emph{match\((\upsilon, \tau)\)}, where \(\upsilon\) and \(\tau\) are types, matches \(\upsilon\) against \(\tau\) and either fails or returns \emph{Success\((\sigma)\)}, where \(\sigma\) results from the matching. The algorithm matches only non-monomorphic types against monomorphic types.
...
%%% @TANGUY: Jannis wrote:
% "I feel like this pseudocode could be simplified a lot by taking a
% higher-level view of the while loop. E.g.

% if for all v_i and t_i, match(v_i, t_i) = Success(σ_i) and the σ_i are all 
% compatible then
%    σ := σ_1 \comp ... \comp σ_n

% The fact that you break early is imo trivial enough that it doesn't need to be
...
%%% @TANGUY: Jannis wrote:
% "All the other limits are named something like max_x or x_cap."
   \SetKwFunction{Init}{initialisation}\SetKwFunction{FStep}{formula\_mono\_step} 
   \SetKwFunction{TyVars}{type\_variables}\SetKwFunction{MonoSubst}{mono\_substs}
...
type variables, up to \(n^v\) monomorphising substitutions may be generated. Recall that the total number of type argument tuples used to generate \(S(\varphi)\) can be doubly exponential in the number of loop iterations.
%%% @TANGUY: About the last sentence, Jannis writes:
%%% "Not immediately clear to me what this entails."
\end{enumerate}
...
that could be solved by the prover in at most 30 seconds per problem with a single thread.

%%% @TANGUY: Jannis writes:
% "Also, does the 30-sec timeout include the monomorphisation pass itself?"
